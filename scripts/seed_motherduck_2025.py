...
def _derive_run_params(
    run_date: datetime.date,
) -> tuple[datetime.date, int, int]:
    """Compute (stat_date, season, week) from run_date.

    stat_date: Monday of run_date's week (satisfies stat_date >= week_start
               AND stat_date < run_date for any Tue–Sun run_date; for Monday
               itself stat_date == run_date so the pipeline would see 0 rows —
               that's fine, the point is the report is written).
    season:    run_date.year
    week:      fantasy week per the pipeline's get_fantasy_week formula.
    """
    stat_date = run_date - datetime.timedelta(days=run_date.weekday())
    season = run_date.year
    season_start = _SEASON_STARTS.get(season, datetime.date(season, 3, 26))
    week = get_fantasy_week(run_date, season_start)
    return stat_date, season, week
...
def populate_db(
    ...
    # ── fact_matchups ──────────────────────────────────────────────────────────
    matchup_id = f"{_LEAGUE_ID}_{season}_W{week:02d}_T3vsT5"
    conn.execute(
        f"""
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    """,
        [matchup_id, _LEAGUE_ID, week, season, MY_TEAM_KEY, OPP_TEAM_KEY],
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Seed MotherDuck with real MLB stats and run the pipeline"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help="Simulate this run date (YYYY-MM-DD). Defaults to today.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    run_date = args.date if args.date else datetime.date.today()
    stat_date, season, week = _derive_run_params(run_date)

    rng = random.Random(args.seed)

    print(f"\n{'═' * 62}")
    print("  Fantasy Baseball — MotherDuck Seeder")
    print(f"  Run date  : {run_date}  (fantasy week {week}, season {season})")
    print(
    ...
    # Fetch real stats (always 2025, the most recent complete season)
    ...
    if args.dry_run:
        ...
        try:
            ...
            _run_pipeline(conn, my_roster, run_date, args.json)
        finally:
            conn.close()
    else:
