...
def populate_db(
    ...
    stat_rows = []
    for _name, mlb_id, *_, positions, _, _ in players:
        player_id = f"422.p.{mlb_id}"
...
def print_report(report: dict[str, Any], conn: duckdb.DuckDBPyConnection) -> None:
    ...
    if matchup:
        ...
        losses = [
            r["category"].upper()
            for r in matchup
            if not r.get("my_leads") and r.get("status") != "toss_up"
        ]
        ...
        for row in matchup:
            ...
            symbol = (
                "✓"
                if row.get("my_leads")
                else ("✗" if status in ("safe_loss", "losing") else "~")
            )
    ...
    if alerts:
        ...
        for a in alerts:
            print(
                f"  ↑ {a.get('player_name', '?')} ({a.get('team', '?')}) — {a.get('days_since_callup', '?')} days ago"
            )
...
def main() -> None:
    ...
    print(f"\n{'═' * 62}")
    print("  2025 Fantasy Baseball — End-to-End Pipeline Test")
    ...
    all_player_ids = conn.execute(f"SELECT player_id FROM {DIM_PLAYERS}").fetchdf()[
        ["player_id"]
    ]
